...
Why this exists: successive drafts of the report claimed a broader verification
than had been performed -- a figure assigned from remembered stdout while being
counted as re-derived, a split computed against a gitignored local database while
reported as parquet-derived, and counts declared un-derivable that were plain row
counts of a shipped table. A claim that a reader can re-execute cannot drift from
...
def main() -> int:
    if not os.path.exists(AUDIT):
        print(f"missing {AUDIT}; run from the repository root", file=sys.stderr)
        return 1
    try:
        import duckdb
    except ImportError:
