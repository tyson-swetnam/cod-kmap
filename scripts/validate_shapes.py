#!/usr/bin/env python3
"""Regression test for schema/shapes/cod-shapes.ttl.
...
EX = "https://tyson-swetnam.github.io/cod-kmap/id/"

HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
ONTOLOGY = ROOT / "schema" / "ontology" / "cod.owl"
SHAPES = ROOT / "schema" / "shapes" / "cod-shapes.ttl"
EXAMPLE = ROOT / "schema" / "shapes" / "cod-example.ttl"

# (focus node local name, SHACL constraint component) -> why it must fire.
# PART B of cod-example.ttl; keep the two in step.
EXPECTED: dict[tuple[str, str], str] = {
    ...
    ("person/bad-double-identity", "SPARQLConstraintComponent"):
        "B4 public id AND codp local identity",
    ...
    ("person/bad-shortcut-only", "SPARQLConstraintComponent"):
        "B9 cod:coauthorOf with no backing cod:CoauthorEdge",
    ("validation/bad-fail-no-detail", "SPARQLConstraintComponent"):
        "B10 Fail verdict with no mismatch detail",
}

# PART A of cod-example.ttl. Any violation on one of these is a false positive:
# the codp local-identity node in particular MUST pass, since the 95 codp:%
# registry rows have no public identifier by construction.
MUST_CONFORM = {
    "person/orcid-0000-0002-1825-0097",
    "person/orcid-0000-0003-1613-5981",
    "person/codp-065ae200269c6713",
    ...
    "validation/A1-orcid",
    "validation/A3-orcid-na",
    "membership/A1-scholar",
    "membership/A3-team",
    "assignment/A1-estuarine",
}
...
def check_pattern_escaping() -> list[str]:
    ...
    # good enough: 'cod-kmap:' also occurs in cod:InternalProvenanceShape's
    # `^cod-kmap:` (a Warning-level detector for the internal provenance form,
    ...
    cases = [
        ...
        # results (no cod-kmap: alternative there — those are machine-generated
        ...
        ("^https?://[^\\s]+$",
         ...
         ["cod-kmap:people", "https://has space/x"]),
    ]
...
def check_sameas_closure() -> list[str]:
    ...
    nodes colliding on a cod:localIdentifier must NOT be, because that value is
    a sha1 of name|orcid|email and same-name people with no ORCID collide.
    """
    import owlrl
    ...
    p = Namespace(EX + "p/")
    g.add((p.reg, RDF.type, COD.RegistryPerson))
    g.add((p.reg, COD.orcidId, Literal("0000-0002-1825-0097")))
    g.add((p.cand, RDF.type, COD.CoauthorCandidate))
    g.add((p.cand, COD.orcidId, Literal("0000-0002-1825-0097")))
    g.add((p.localA, RDF.type, COD.RegistryPerson))
    g.add((p.localA, COD.localIdentifier, Literal("065ae200269c6713")))
    g.add((p.localB, RDF.type, COD.RegistryPerson))
    g.add((p.localB, COD.localIdentifier, Literal("065ae200269c6713")))
    ...
    if same(p.localA, p.localB):
        problems.append(
            "cod:localIdentifier collision produced owl:sameAs — two same-name "
            "COD-internal people were merged; cod:localIdentifier must not be "
