...
"""Capped, quota-aware coastal-works harvest for the COD registry.

Design constraints measured live, not assumed:
...
WORKS_OUT = "harvest_works.parquet"
PAIRS_OUT = "harvest_pairs.parquet"
...
def append_parquet(path, rows, batch_index=None):
    ...
    The earlier implementation read the whole file, concatenated, and rewrote
    it. That is a read-modify-write on a growing file: a reader (or a second
    harvest process) that opens it mid-write sees truncated bytes and fails
    with "Parquet magic bytes not found in footer" — observed once at 19.5 MB.
    Per-batch shards are write-once, so no reader ever sees a partial file and
    the cost per batch stops growing with the corpus. Consumers read the
    directory as a dataset: pq.read_table('harvest_pairs.parquet.d').
    """
    if not rows:
        return
    d = path + ".d"
...
def harvest(shards_df, topic_ids, per_author_cap=200, batch_size=50,
            request_budget=6500, progress_every=10):
    """Harvest coastal works for every author in shards_df.
    ...
    """
    st = load_state()
    ids = [str(v).rsplit("/", 1)[-1] for v in shards_df.openalex_id]
    ...
    for bi, batch in enumerate(batches):
        ...
        # already paid for: measured 1,000 duplicate works and 5 wasted
        # requests on a 2-run test before this was stored.
        ...
        while pages < max_pages:
            ...
            # requests past the limit (measured: 50 spent against a
            # budget of 6 when the check was per-batch only).
            if st["requests_used"] >= request_budget:
                break
            d = oah.oa_get(
                "works",
